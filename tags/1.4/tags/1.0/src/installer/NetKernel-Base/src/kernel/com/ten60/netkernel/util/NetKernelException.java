 ...
 *
 * File:          $RCSfile: NetKernelException.java,v $
 * Version:       $Name:  $ $Revision: 1.12 $
 * Last Modified: $Date: 2007/10/15 15:44:23 $
 *****************************************************************************/
 ...
 * @author  tab
 */
public class NetKernelException extends java.lang.Exception implements IXMLException
{
	...
	{	String result=null;
		Throwable deepest = getDeepest();
		if (deepest instanceof NetKernelException)
		{	result = ((NetKernelException)deepest).getId();
		}
		else if (deepest instanceof Throwable)
		{	result = deepest.getClass().getName();
		}
		return result;
	}
	...
	{	String result=null;
		Throwable deepest = getDeepest();
		if (deepest instanceof Throwable)
		{	result = deepest.getMessage();
		}
		return result;
	}
	
	private Throwable getDeepest()
	{	Throwable result=null;
		if (mCauses!=null)
		{	for (Iterator i=mCauses.iterator(); i.hasNext(); )
			{	Throwable t = (Throwable)i.next();
				if (t instanceof NetKernelException)
				{	result = ((NetKernelException)t).getDeepest();
				}
				else
				{	result = t;
				}
	...
	{	aWriter.write("<ex>");
		...
		if (getMessage()!=null) XMLUtils.writeEscaped(aWriter,"message",getMessage());
		if (mRequestId!=null) XMLUtils.writeEscaped(aWriter,"requestid",mRequestId);
	...
	public static void writeThrowable(Throwable aThrowable, Writer aWriter) throws IOException
	{
		aWriter.write("<ex>");
		XMLUtils.write(aWriter, "id", aThrowable.getClass().getName());
		if (aThrowable.getMessage()!=null)
		{	XMLUtils.writeEscaped(aWriter,"message",aThrowable.getMessage());
		}
		XMLUtils.writeStack(aWriter,aThrowable.getStackTrace(),sTraceDepth);
		Throwable cause = aThrowable.getCause();
		if (cause!=null)
		{	if (cause instanceof IXMLException)
			{	((IXMLException)cause).appendXML(aWriter);
			}
			else
			{	writeThrowable(cause,aWriter);
			}
		}
		aWriter.write("</ex>");
	}
